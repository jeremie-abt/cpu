...
func handleParseMov(l lexer.Lexer) ([]Node, error) {
	...
	if destination.Type == lexer.TokenOpenBracket {
		response, err := parseMemoryOperand(l)
		if err != nil {
			return nil, err
		}

		operands = append(operands, response)
	} else if destination.Type == lexer.TokenRegister {
