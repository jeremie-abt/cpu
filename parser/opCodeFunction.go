...
// opCodeHandler are parsing OpCode functions that handle the parsing of all OpCode.
type opCodeHandler func(lexer.Lexer) ([]Node, error)

// handleMov is the MOV parser handler, it supposes that the lexer is in the state just after
// having read the MOV instruction, so we are parsing mov parameters now.
func handleMov(l lexer.Lexer) ([]Node, error) {
	...
	if source == nil || destination == nil || comma == nil {
		return nil, &parsingError{
